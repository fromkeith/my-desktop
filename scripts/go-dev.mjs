...
async function main() {
  try {
    ...
    // Get the arguments to forward to `air`.
    const airArgs = process.argv.slice(2);
    ...
    console.log(`> air ${airArgs.join(' ')}`);

    // Execute the `air` command.
    const airProcess = spawn(command, args, {
      ...
      stdio: 'inherit',
      // Run in a shell to ensure `air` is found in PATH on all systems.
      shell: true,
    });
    ...
    airProcess.on('error', (err) => {
        console.error('Failed to start the air process.', err);
        process.exit(1);
    });

  } catch (error) {
    console.error('An error occurred in the dev script:', error);
    process.exit(1);
  }
}
